...
def _hermetic_batch(monkeypatch):
    ...
    monkeypatch.setattr(cli.evaluation, "_ledger_line_count", lambda: 0)
    monkeypatch.setattr(cli.evaluation, "_ledger_cost_since", lambda start: 0.0)
    monkeypatch.setattr(cli.evaluation, "_ledger_spend_since", lambda start: (0.0, 0, 0))
...
def test_batch_stops_launching_jobs_over_budget(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli.evaluation, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli.evaluation, "_ledger_cost_since", lambda start: 99.0)
    monkeypatch.setattr(cli.evaluation, "_ledger_spend_since", lambda start: (99.0, 3, 3))
    targets = _write_targets(
...
def test_batch_stops_when_spend_is_not_being_recorded(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli.evaluation, "_ledger_spend_since", lambda start: (0.0, 3, 0))
    targets = _write_targets(
        tmp_path, "\n".join(f"https://example.com/{n}" for n in range(4)) + "\n"
    )
    result = runner.invoke(
...
def test_ledger_spend_only_counts_premium_records(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli.evaluation, "_ledger_path", lambda: ledger)
...
def test_batch_stops_when_only_some_premium_calls_report_cost(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli.evaluation, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli.evaluation, "_ledger_spend_since", lambda start: (0.03, 2, 1))
    targets = _write_targets(
        tmp_path, "\n".join(f"https://example.com/{n}" for n in range(4)) + "\n"
    )
    result = runner.invoke(
...
def test_batch_runs_on_when_every_premium_call_is_priced(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli.evaluation, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli.evaluation, "_ledger_spend_since", lambda start: (0.06, 2, 2))
    targets = _write_targets(
