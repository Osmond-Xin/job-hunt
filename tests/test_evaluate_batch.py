...
def _hermetic_batch(monkeypatch):
    ...
    monkeypatch.setattr(cli, "_batch_preflight", lambda budget_enforced=False: None)
    monkeypatch.setattr(cli, "_partition_already_evaluated", lambda targets: (targets, []))
    monkeypatch.setattr(cli, "_ledger_line_count", lambda: 0)
    monkeypatch.setattr(cli, "_ledger_cost_since", lambda start: 0.0)
    monkeypatch.setattr(cli, "_ledger_spend_since", lambda start: (0.0, 0, 0))
...
def test_batch_rejects_an_empty_file(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: _FakeGraph())
...
def test_batch_stops_launching_jobs_over_budget(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli, "_ledger_cost_since", lambda start: 99.0)
    monkeypatch.setattr(cli, "_ledger_spend_since", lambda start: (99.0, 3, 3))
    targets = _write_targets(
...
def test_batch_surfaces_withheld_artifacts(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: _WarningGraph())
    targets = _write_targets(tmp_path, "https://example.com/1\n")
    result = runner.invoke(app, ["evaluate-batch", str(targets)])
    assert result.exit_code == 0, result.output
...
def test_batch_counts_provider_outages_as_failures(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: _DegradedGraph())
    targets = _write_targets(tmp_path, "https://example.com/1\n")
    result = runner.invoke(app, ["evaluate-batch", str(targets)])
...
def test_preflight_rejects_an_empty_premium_command(monkeypatch) -> None:
    monkeypatch.setattr(cli, "load_settings", lambda: _stub_settings(command=[]))
    monkeypatch.setenv("JOB_HUNT_SKIP_CV_SYNC_CHECK", "1")
...
def test_budget_preflight_uses_the_providers_own_json_detection(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        cli, "load_settings", lambda: _stub_settings(command=["claude", "-p", "json"])
    )
    with pytest.raises(typer.Exit):
...
def test_budget_preflight_requires_the_ledger(monkeypatch) -> None:
    monkeypatch.setenv("JOB_HUNT_SKIP_CV_SYNC_CHECK", "1")
    monkeypatch.setattr(cli.shutil, "which", lambda name: "/usr/bin/claude")
    monkeypatch.setattr(cli, "load_settings", lambda: _stub_settings(ledger_enabled=False))
    with pytest.raises(typer.Exit):
        _REAL_PREFLIGHT(budget_enforced=True)
...
def test_batch_stops_when_spend_is_not_being_recorded(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(cli, "_ledger_spend_since", lambda start: (0.0, 3, 0))
    targets = _write_targets(
        tmp_path, "\n".join(f"https://example.com/{n}" for n in range(4)) + "\n"
    )
    result = runner.invoke(
...
def test_batch_stops_when_only_some_premium_calls_report_cost(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli, "_ledger_spend_since", lambda start: (0.03, 2, 1))
    targets = _write_targets(
        tmp_path, "\n".join(f"https://example.com/{n}" for n in range(4)) + "\n"
    )
    result = runner.invoke(
...
def test_batch_runs_on_when_every_premium_call_is_priced(tmp_path, monkeypatch) -> None:
    ...
    graph = _FakeGraph()
    monkeypatch.setattr(cli, "build_evaluate_job_graph", lambda: graph)
    monkeypatch.setattr(cli, "_ledger_spend_since", lambda start: (0.06, 2, 2))
    targets = _write_targets(
